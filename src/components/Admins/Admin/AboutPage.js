...
import { createAbout, deleteAbout, updateAbout } from '../../../actions/About';
...
const Form = () => {
  ...
  const AboutUpdate = useSelector((state) => state.About.filter(A => A._id === currentIdAbout)[0]);
  const dispatch = useDispatch();
  useEffect(() => {
    dispatch(getAbouts());   
      // by putting the name in [ ] in every update the currentid is change 
  }, [currentIdAbout, dispatch]);
  useEffect(() => {
    if (AboutUpdate){
