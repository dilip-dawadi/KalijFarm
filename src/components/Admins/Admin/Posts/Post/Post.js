...
import { deleteKalij } from '../../../../../actions/kalijs';
    ...
    return (
        ...
        <div className={classes.overlay2}>
          <Button style={{ color: 'white' }} size="small" disabled={!user?.result} onClick={(e) =>
              {e.stopPropagation();
               setCurrentId(kalij._id)}}><MoreHorizIcon fontSize="medium" /></Button>
        </div>
        ...
        <CardContent>
          <Typography variant="body2" color="textSecondary" component="p">{kalij.message}</Typography>
        </CardContent>
        </ButtonBase>
        <CardActions className={classes.cardActions}>
          ...
          <Button size="small" color="primary" onClick={() => dispatch(deleteKalij(kalij._id))}><DeleteIcon fontSize="small" /> Delete</Button>
        </CardActions>
      </Card>
